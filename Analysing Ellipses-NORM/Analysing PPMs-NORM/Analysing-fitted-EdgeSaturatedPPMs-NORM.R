...
spp.ppms <- lapply(1:2500, function(x){
      readRDS(paste0("../Resultados/Analysis-centroids/Fitted-Edge-Saturated-PPMs-NORM/PPM-", x, ".rds"))
})
...
df.results <- data.frame(df.centroids, vars.spp, approach = "PPM-sat-NORM", centr.conf = "edge")

write.csv(df.results, "Simulated-species/Results-EdgeSaturatedPPMs-NORM.csv", row.names = F)
