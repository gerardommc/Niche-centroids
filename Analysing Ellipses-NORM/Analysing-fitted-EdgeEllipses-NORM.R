...
ellips <- lapply(1:2500, function(x){
      readRDS(paste0("../Resultados/Analysis-centroids/Fitted-Edge-Ellipses-NORM/Ellips-", x, ".rds"))
})
...
df.results <- data.frame(df.centroids, vars.spp, approach = "Ellipses-NORM", centr.conf = "edge")

write.csv(df.results, "Simulated-species/Results-EdgeEllipses-NORM.csv", row.names = F)
