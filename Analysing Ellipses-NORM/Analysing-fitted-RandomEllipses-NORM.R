...
ellips <- lapply(1:2500, function(x){
      readRDS(paste0("../Resultados/Analysis-centroids/Fitted-Random-Ellipses-NORM/Ellips-", x, ".rds"))
})
...
df.results <- data.frame(df.centroids, vars.spp, approach = "Ellipses-NORM", centr.conf = "random")

write.csv(df.results, "Simulated-species/Results-RandomEllipses-NORM.csv", row.names = F)
