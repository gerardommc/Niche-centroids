...
spp.ppms <- lapply(1:2500, function(x){
      readRDS(paste0("../Resultados/Analysis-centroids/Fitted-Outer-Saturated-PPMs-NORM/PPM-", x, ".rds"))
})
...
df.results <- data.frame(df.centroids, vars.spp, approach = "PPM-sat-NORM", centr.conf = "outer")

write.csv(df.results, "Simulated-species/Results-OuterSaturatedPPMs-NORM.csv", row.names = F)
