...
spp.ppms <- lapply(1:2500, function(x){
      readRDS(paste0("../Resultados/Analysis-centroids/Fitted-Random-Saturated-PPMs-norm/PPM-", x, ".rds"))
})
...
df.results <- data.frame(df.centroids, vars.spp, approach = "PPM-sat-NORM", centr.conf = "random")

write.csv(df.results, "Simulated-species/Results-RandomSaturatedPPMs-NORM.csv", row.names = F)
